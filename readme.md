...
- **資料處理**：Pandas, tqdm
- **NLP**：KeyBERT（關鍵字抽取），BERT（情緒分析），NER（股票代號/公司名稱識別）
...
```
PTT Stock 板 / 鉅亨網 / Reddit（增量） / Reddit 歷史（Arctic Shift）
        ↓
  scrapers/（ptt_scraper、cnyes_scraper、reddit_scraper、reddit_batch_loader）
        ↓
...
└── us_stock_prices   ← VOO（yfinance）
  ...
  DW（Snowflake Schema）
  ├── dim_market / dim_date / dim_source / dim_stock
  ├── fact_sentiment
  └── Data Mart
      ├── mart_daily_summary（儀表板用）
      └── mart_hot_stocks（API 用，partial index）
        ↓
...
- [x] Phase 4：Data Mart（mart_daily_summary / mart_hot_stocks + partial index）
...
- [x] Phase 4：Looker Studio CSV 匯出（daily_sentiment / hot_articles / source_stats）
...
- [x] Phase 5：NER 命名實體識別（regex + 公司名稱最長匹配）
