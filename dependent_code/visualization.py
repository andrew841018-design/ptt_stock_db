...
from plt_function import (
    ...
    plot_sentiment_and_price_trend,
    plot_sentiment_avg_by_source_bar,
)
from config import (
...
# ─── Sentiment by Source（當期平均橫條圖；per-source 趨勢請用 sidebar Source Filter）─
if len(df['Source'].unique()) > 1:
    st.subheader("Sentiment by Source")
    st.pyplot(plot_sentiment_avg_by_source_bar(df))
...
for market_code, market_name in _MARKETS:
    ...
    # 用 sidebar 日期過濾，並重新歸一化到選定區間起點 = 1.0
    mask_pred = (
        (df_pred["date"] >= pd.Timestamp(start_date)) &
        (df_pred["date"] <= pd.Timestamp(end_date))
    )
    df_view = df_pred[mask_pred].copy()
    if df_view.empty:
        st.info(f"{market_name}：所選日期區間無預測資料")
        continue

    strat_base = df_view["strategy_cumulative_return"].iloc[0]
    bnh_base   = df_view["buy_and_hold_return"].iloc[0]
    df_view["strategy_cumulative_return"] = df_view["strategy_cumulative_return"] / strat_base
    df_view["buy_and_hold_return"]        = df_view["buy_and_hold_return"] / bnh_base

    accuracy           = (df_view["true"] == df_view["pred"]).mean()
    final_strategy     = df_view["strategy_cumulative_return"].iloc[-1]
    final_buy_and_hold = df_view["buy_and_hold_return"].iloc[-1]
    ...
    chart_df = df_view.set_index("date")[["strategy_cumulative_return", "buy_and_hold_return"]]
