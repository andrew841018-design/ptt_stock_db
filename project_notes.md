...
- **TIMESTAMP DEFAULT 的時區陷阱**：`DEFAULT NOW()` 用 PG session 時區（容器設定為 Asia/Taipei），但 Python `datetime.utcnow()` 是 UTC。混用會讓「同一欄位、不同 row 不同時區」，後續算 `hours_ago` 一定錯。原則：**全 UTC 或全 PG 本地，二選一**。`articles.scraped_at` 已對齊 UTC（`DEFAULT (NOW() AT TIME ZONE 'UTC')` + scraper 寫入 `datetime.utcnow()`）；`calculated_at` / `labeled_at` / `run_at` 三個內部一致用 PG NOW()，不混用。
...
## 38. PEP 585 vs PEP 604 — Python 3.9 type hint 相容性

**PEP 585（Generic Alias）**：自 **Python 3.9** 起，內建容器型別（`list` / `dict` / `tuple` / `set` / `frozenset` / `type`）支援下標泛型語法：

```python
def f(x: list[str]) -> dict[str, int]: ...   # ✅ Python 3.9+ OK
```

不需 `from typing import List, Dict`，runtime / type-check 均 OK。

**PEP 604（Union Types）**：自 **Python 3.10** 起，可用 `|` 取代 `Union[...]` / `Optional[...]`：

```python
def g(x: str | None) -> int | float: ...   # ❌ Python 3.10+ only
```

Python 3.9 拋 `TypeError: unsupported operand type(s) for |: 'type' and 'NoneType'`；3.9 必須用 `Optional[X]` / `Union[X, Y]`。

**本專案規則**：本機 conda env Python 3.9，**可以**用 `list[X]` / `dict[X, Y]`（PEP 585），**不能**用 `X | None`（PEP 604）。

**踩過的坑**：04-30 code review 將 `list[str]` 誤判為 PEP 604 incompatibility，實際上是 PEP 585 在 3.9 已支援。確認方法：直接 import 驗證，不要靠經驗推斷。

---

## 39. Refactor 改名時要同步更新測試 mock target

**事件（2026-04-30）**：`api.py` 早已從 `get_pg_readonly` refactor 為連線池版 `get_pg_pooled`（`from pg_helper import init_pool, get_pg_pooled`），但 `test_api.py` 仍 patch `api.get_pg_readonly`，14 個測試全部 FAIL：

```
AttributeError: <module 'api'> does not have the attribute 'get_pg_readonly'
```

**根因**：`unittest.mock.patch` 在 patch 不存在的 attribute 時會直接 raise，不是靜默 fallthrough。

**修法**：grep 整個 codebase 找 `api.get_pg_readonly` → 全改成 `api.get_pg_pooled`。

**通用 checklist（refactor 改名前必跑）**：

```bash
# 1. grep 所有引用
grep -rn "old_name" --include="*.py"

# 2. 特別注意 test_*.py 的 patch() target
grep -rn 'patch("[a-z_]*\.old_name"' --include="test_*.py"
