...
def check_db_and_sources() -> tuple[bool, str, dict]:
    ...
    """
    try:
        # 顯式從絕對路徑載 dependent_code/config.py，alias 進 sys.modules['config']，
        # 避免 line_bot/config.py（依賴 pydantic_settings）在某些 race 下被當成 'config' 命中。
        # 跟 send_discord() 用 importlib.util 載 notify_discord 同樣手法。
        import importlib.util

        config_path = DEPENDENT_CODE / "config.py"
